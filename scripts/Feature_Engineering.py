...
class DataProcess:
    def __init__(self):
        self.sep_index = ['BaseExcess', 'HCO3', 'FiO2', 'pH', 'PaCO2', 'SaO2', 'AST',
                          'BUN', 'Alkalinephos', 'Calcium', 'Chloride', 'Creatinine',
                          'Glucose', 'Lactate', 'Magnesium', 'Phosphate', 'Potassium',
                          'Bilirubin_total', 'Hct', 'Hgb', 'PTT', 'WBC', 'Platelets']
        self.con_index = ['HR', 'O2Sat', 'Temp', 'SBP', 'MAP', 'DBP', 'Resp', 'EtCO2']
    ...
    def feature_empiric_score(self, dat):
        """
    ...
    def feature_extraction(self, case):
        ...
        temp_data = self.feature_informative_missingness(pid, self.con_index + self.sep_index)
        ...
        feature_B = self.feature_slide_window(feature_A, [0, 1, 3, 4, 6])
        # 8 empiric features
        feature_C = self.feature_empiric_score(feature_A)
        # A total of 168 features were obtained
        features = np.column_stack((feature_A, feature_B, feature_C))

        return features, labels

    def data_process(self, data_set, data_path_dir):
        """
        ...
        for psv in data_set:
            ...
            features, labels = self.feature_extraction(patient)
